 ...
 */
    ...
    hasMajorDiagonalConflictAt: function(majorDiagonalColumnIndexAtFirstRow, row = 0) {
      ...
      for (var i = 0; i < this.rows().length; i++) {
        for (var j = majorDiagonalColumnIndexAtFirstRow; j < this.rows().length; j++) {
          if (this.rows()[i][j] === 1 && this._getFirstRowColumnIndexForMajorDiagonalOn(i, j) === offset) {
            count++;
            if (count > 1) {
              return true;
            }
    ...
    hasMinorDiagonalConflictAt: function(minorDiagonalColumnIndexAtFirstRow, row = 0) {
      ...
      for (var i = 0; i < this.rows().length; i++) {
        for (var j = minorDiagonalColumnIndexAtFirstRow; j >= 0; j--) {
          if (this.rows()[i][j] === 1 && this._getFirstRowColumnIndexForMinorDiagonalOn(i, j) === offset) {
            count++;
            if (count > 1) {
              return true;
            }
    ...
    hasAnyMinorDiagonalConflicts: function() {
      ...
      for (var j = 1; j < this.rows().length; j++) {
        if (this.hasMinorDiagonalConflictAt(this.get('n') - 1, j)) {
          return true;
        }
      }
      return false; // fixme
